...
namespace Memcury
{
    ...
    public:
        ...
        template <int range>
        auto ScanForRanged(std::vector<uint8_t> opcodesToFind, bool forward = true, int toSkip = 0) -> Scanner;

        auto ScanFor(std::vector<uint8_t> opcodesToFind, bool forward = true, int toSkip = 0) -> Scanner { return ScanForRanged<2048>(opcodesToFind, forward, toSkip); }
        auto ScanForShortRange(std::vector<uint8_t> opcodesToFind, bool forward = true, int toSkip = 0) -> Scanner { return ScanForRanged<128>(opcodesToFind, forward, toSkip); }
        auto ScanForLongRange(std::vector<uint8_t> opcodesToFind, bool forward = true, int toSkip = 0) -> Scanner { return ScanForRanged<16384>(opcodesToFind, forward, toSkip); }
        ...
        auto Jump() -> Scanner { _address.Jump(); return *this; }
        auto RelativeOffset(uint32_t offset) -> Scanner { _address.RelativeOffset(offset); return *this; }
        auto AbsoluteOffset(uint32_t offset) -> Scanner { _address.AbsoluteOffset(offset); return *this; }
    ...
    auto Scanner::ScanForRanged(std::vector<uint8_t> opcodesToFind, bool forward, int toSkip) -> Scanner
    {
        ...
        for (auto i = (forward ? 1 : -1); forward ? (i < range) : (i > -range); forward ? i++ : i--)
        {
            ...
            if (found)
            {
                ...
                if (toSkip != 0)
                {
                    return ScanForRanged<range>(opcodesToFind, forward, toSkip - 1);
                }
    ...
    template <typename T = const wchar_t*>
    auto Scanner::FindStringRef(T string, bool find_first) -> Scanner
    {
